...
@implementation UIView (KNFadeAnimation)
...
{
    intervalDuration = MAX(KNMINDURATION * KNMULTIPLED, MIN(self.fadeAnimationDuration * KNMULTIPLED, intervalDuration));
...
{
    fadeAnimationDuration = MAX(KNMINDURATION, MIN(KNMAXDURATION, fadeAnimationDuration));
    if (self.intervalDuration > fadeAnimationDuration * KNMULTIPLED || self.intervalDuration <= 0) {
        self.intervalDuration = fadeAnimationDuration * KNMULTIPLED;
    }
...
- (void)animateFadeWithComplete: (void (^)(void))complete
{
    ...
    self.isFading = YES;
    if (self.fadeAnimationDuration <= 0) { self.fadeAnimationDuration = (KNMAXDURATION + KNMINDURATION) / 2; }
    if (self.intervalDuration <= 0) { self.intervalDuration = self.fadeAnimationDuration * KNMULTIPLED; }
