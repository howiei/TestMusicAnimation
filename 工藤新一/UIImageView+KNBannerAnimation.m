//
//  UIImageView+KNBannerAnimation.m
//  工藤新一
//
...
@implementation UIImageView (KNBannerAnimation)
...
- (void)setFadeDuration: (NSTimeInterval)fadeDuration
{
    if (self.bannerImages.count > 1) {
        ...
        self.fadeAnimationDuration = fadeDuration / (self.bannerImages.count + 3) / KNMULTIPLED;
        self.intervalDuration = self.fadeAnimationDuration * KNMULTIPLED;
    }
}
...
- (void)associatePageControlWithCurrentIdx: (NSInteger)idx
{
    ...
    if (!pageControl) {
        pageControl = [[UIPageControl alloc] initWithFrame: CGRectMake(self.frame.origin.x, CGRectGetHeight(self.frame) - 27 + self.frame.origin.y, CGRectGetWidth(self.frame), 37)];
